...
import org.kdvcs.klux.worldgen.portal.ModTeleporter;
...
public class PortalBlock extends Block {
    ...
    private void handleKaupenPortal(Entity player, BlockPos pPos) {
        if (player.level() instanceof ServerLevel serverlevel) {
            ...
            ResourceKey<Level> resourcekey = player.level().dimension() == ModDimensions.SPECTRUM_LEVEL_KEY ?
                    Level.OVERWORLD : ModDimensions.SPECTRUM_LEVEL_KEY;

            ServerLevel portalDimension = minecraftserver.getLevel(resourcekey);
            if (portalDimension != null && !player.isPassenger()) {
                if(resourcekey == ModDimensions.SPECTRUM_LEVEL_KEY) {
