...
public class FluxSynthesizerScreen extends AbstractContainerScreen<FluxSynthesizerMenu> {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(Klux.MODID,"textures/gui/gui_flux_synthesizer.png");

    private FluidTankRenderer renderer;
    private FluidTankRenderer rendererOutput;

    public FluxSynthesizerScreen(FluxSynthesizerMenu menu, Inventory inventory, Component component) {
    ...
    private void assignFluidRenderer() {
        renderer = new FluidTankRenderer(32000, true, 24, 33);
        rendererOutput = new FluidTankRenderer(32000,true,24,33);
    }
    ...
    private void renderFluidAreaTooltips(GuiGraphics guiGraphics, int mouseX, int mouseY, int x, int y) {
        ...
        FluidStack outputFluidStack = menu.getOutputFluidStack();

        if (isMouseAboveArea(mouseX, mouseY, x, y, 12, 43) && !fluidStack.isEmpty()) {
        ...
        if (isMouseAboveArea(mouseX, mouseY, x, y, 140, 43) && !outputFluidStack.isEmpty()) {
    ...
    private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
        if(menu.isCrafting()) {
            guiGraphics.blit(TEXTURE, x + 58, y + 38, 176, 0, menu.getScaledProgress(), 14);
        }
    }
