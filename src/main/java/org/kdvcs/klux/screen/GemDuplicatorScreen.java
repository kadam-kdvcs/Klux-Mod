...
public class GemDuplicatorScreen extends AbstractContainerScreen<GemDuplicatorMenu> {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(Klux.MODID,"textures/gui/gui_gem_duplicator.png");

    private FluidTankRenderer renderer;

    public GemDuplicatorScreen(GemDuplicatorMenu menu, Inventory inventory, Component component) {
    ...
    private void assignFluidRenderer() {
        renderer = new FluidTankRenderer(64000, true, 36, 61);
    }
    ...
    private void renderFluidAreaTooltips(GuiGraphics guiGraphics, int mouseX, int mouseY, int x, int y) {
        if (isMouseAboveArea(mouseX, mouseY, x, y, 12, 17)) {
    ...
    private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
        if(menu.isCrafting()) {
            guiGraphics.blit(TEXTURE, x + 95, y + 42, 176, 0, menu.getScaledProgress(), 8);
        }
    }
