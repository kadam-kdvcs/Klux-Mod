...
public class LiquidFilterScreen extends AbstractContainerScreen<LiquidFilterMenu> {
    ...
    public LiquidFilterScreen(LiquidFilterMenu menu, Inventory inventory, Component component) {
    ...
    private void renderFluidAreaTooltips(GuiGraphics guiGraphics, int mouseX, int mouseY, int x, int y) {
        if (isMouseAboveArea(mouseX, mouseY, x, y, 52, 17)) {
    ...
    private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
        if(menu.isCrafting()) {
            guiGraphics.blit(TEXTURE, x + 99, y + 39, 176, 0, menu.getScaledProgress(), 17);
        }
    }
